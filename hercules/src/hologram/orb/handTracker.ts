/**
 * HERCULES · MediaPipe hand-gesture tracker (ported from ultron_hologram)
 * -----------------------------------------------------------------------
 * Detects pinch gestures over the live camera feed and maps them to orb
 * controls: one pinched hand drags (spin), two pinched hands pinch/expand
 * (zoom). The overlay can be recolored live to match the active theme via
 * `setPalette`.
 */
import {
  FilesetResolver,
...
export class HandTracker {
  ...
  // Theme-aware overlay colors; updated via setPalette
  private activeGlow = "rgba(255,170,48,0.5)";
  private activeHot = "rgba(255,170,48,0.7)";
  private activePinch = "#ffcc66";
  ...
  /** Re-tint the hand overlay to match the active theme accent (hex string). */
  setPalette(accent: string): void {
    const { r, g, b } = hexToRgb(accent);
    const a = (v: number) => clamp01(v);
    this.activeGlow = `rgba(${r},${g},${b},${a(0.5)})`;
    this.activeHot = `rgba(${r},${g},${b},${a(0.7)})`;
    this.activePinch = `rgba(${Math.min(255, r + 60)},${Math.min(255, g + 60)},${Math.min(255, b + 30)},1)`;
  }
  ...
  private drawOverlay(landmarks: NormalizedLandmark[][]): void {
    ...
    for (const lm of landmarks) {
      ...
      ctx.strokeStyle = pinched ? this.activePinch : this.activeGlow;
      ...
      ctx.fillStyle = pinched ? this.activePinch : this.activeHot;
      for (const [x, y] of [
        [tx, ty],
        [ix, iy],
      ]) {
...
function hexToRgb(hex: string): { r: number; g: number; b: number } {
  ...
  if (Number.isNaN(n)) return { r: 255, g: 170, b: 48 };
